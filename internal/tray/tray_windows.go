...
// icoBytes is the Shelf tray icon, generated by `go run ./cmd/gen-icons`.
// Embedding the ICO into the binary keeps the runtime dependency to
// zero external files — the tray works even if the data directory is
// empty. The file holds 16/32/48 PNG sub-images; Windows picks the
// closest match for the current DPI when the tray renders.
//
//go:embed icon.ico
var icoBytes []byte
...
func run(cfg Config) error {
	...
	// Embedded .ico → HICON. Falls back to the stock app icon if the
	// embedded blob fails to parse (shouldn't happen in a built binary,
	// but a guaranteed-visible tray is worth a belt-and-suspenders).
	hIcon, iconErr := loadEmbeddedIcon()
	if iconErr != nil {
		stock, _, stockErr := procLoadIconW.Call(0, uintptr(idiApplication))
		if stock == 0 {
			return fmt.Errorf("tray: LoadIcon fallback: %v (embed error: %w)", stockErr, iconErr)
		}
		hIcon = stock
	}
...
// loadEmbeddedIcon turns the embedded icon.ico bytes into an HICON
// sized to match the system-tray "small icon" metric. Uses the
// LookupIconIdFromDirectoryEx + CreateIconFromResourceEx pair — the
// standard Win32 idiom for consuming an ICO blob that isn't backed by
// a file or a PE resource.
//
// Returns a non-zero HICON on success. On any parse/syscall failure,
// returns an error so the caller can fall back to IDI_APPLICATION.
func loadEmbeddedIcon() (uintptr, error) {
	if len(icoBytes) < 6 {
		return 0, fmt.Errorf("tray: embedded icon too small (%d bytes)", len(icoBytes))
	}
	cx, cy := smallIconSize()
	cxArg := uintptr(cx) // #nosec G115 -- small-icon metric, always > 0.
	cyArg := uintptr(cy) // #nosec G115 -- small-icon metric, always > 0.

	// #nosec G103 -- syscall pointer to a global []byte slice; the slice
	// header backs stable memory for the duration of the call.
	base := uintptr(unsafe.Pointer(&icoBytes[0]))

	// LookupIconIdFromDirectoryEx(presbits, fIcon=TRUE, cxDesired, cyDesired,
	//   Flags=LR_DEFAULTCOLOR) → offset into the blob of the chosen entry.
	offset, _, err := procLookupIconIdFromDirectoryEx.Call(
		base,
		1, // fIcon = TRUE
		cxArg,
		cyArg,
	...
	if offset == 0 {
		return 0, fmt.Errorf("tray: LookupIconIdFromDirectoryEx: %w", err)
	}
	// The offset is into the same buffer; bounds-check before slicing.
	// The Win32 API returns a non-negative offset that fits the 32-bit
	// Windows HANDLE-sized uintptr; our own icoBytes is a few KiB at
	// most. The cast is safe under those invariants.
	off := int(offset) // #nosec G115 -- bounds-checked below against a few-KiB buffer.
	if off < 0 || off >= len(icoBytes) {
		return 0, fmt.Errorf("tray: icon offset %d out of range (%d)", off, len(icoBytes))
	}
	sizeArg := uintptr(len(icoBytes) - off) // #nosec G115 -- len-off is non-negative by the bounds check above.

	// CreateIconFromResourceEx(presbits, dwResSize, fIcon=TRUE,
	//   dwVer=0x00030000, cxDesired, cyDesired, Flags=LR_DEFAULTCOLOR).
	// #nosec G103 -- syscall pointer into the same stable []byte slice.
	hIcon, _, err := procCreateIconFromResourceEx.Call(
		uintptr(unsafe.Pointer(&icoBytes[off])),
		sizeArg,
		1,          // fIcon = TRUE
...
// smallIconSize returns (cx, cy) for the "small icon" metric (SM_CXSMICON,
// SM_CYSMICON) — what the system tray and window title bar use. Falls
// back to 16x16 if the metric lookup fails.
func smallIconSize() (int32, int32) {
	cx, _, _ := procGetSystemMetrics.Call(uintptr(smCXSmIcon))
	cy, _, _ := procGetSystemMetrics.Call(uintptr(smCYSmIcon))
	if cx == 0 || cy == 0 {
		return 16, 16
	}
	// System metrics for icon sizes fit an int32 on every supported
	// Windows version; the clamp below keeps us safe on any future
	// high-DPI variant too.
	...
	return int32(cx), int32(cy) // #nosec G115 -- clamped to int32 max above.
}
