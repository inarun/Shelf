...
4. **Localhost-only by default.** Network bind address is `127.0.0.1` unless the user explicitly configures otherwise. Even then, warn loudly.
   ...
   > **2026-04-22 (Session 21 — v0.3.2):** `POST /api/shutdown` is the only mutating endpoint that additionally asserts `net.IP.IsLoopback()` on `r.RemoteAddr`, above and beyond the Host middleware's Host-header allowlist. Belt-and-suspenders: a future widening of the Host allowlist (planned for v0.6 Tailscale work) must not accidentally expose shutdown. This pattern generalises via the new §Conventions bullet on destructive endpoints.
...
- `providers/metadata` and `providers/reading` are interfaces. Adding Hardcover is a new package implementing the interface, not a modification of existing code.
...
5. **Metadata provider** (Open Library, Hardcover) — lowest, used only for pure metadata (pages, publisher, cover) not personal data
...
A single zero-sized `<svg class="icon-sprite">` holds every icon as a `<symbol>`. The `nav` partial emits the `iconSprite` partial once per page so consumers can write `<svg class="icon"><use href="#icon-..."/></svg>` and inherit color via `currentColor`. Current symbols (13):

- **Brand:** `icon-logo` (Session 10) — a 24×24 bookshelf mark paired with the wordmark in the nav.
- **Actions:** `icon-star-filled`, `icon-star-outline`, `icon-star-half`, `icon-plus`, `icon-x`, `icon-search`, `icon-keyboard`, `icon-power` (Session 21 — nav Quit button + full-viewport "Shelf is shutting down" state).
- **Provenance:** `icon-audio` (Session 14) — Audiobookshelf timeline-entry badge.
...
- **Manual-entry form (Session 23):** `.add-manual` (section wrapper) → `.add-manual-form` (flex-column container) → `.add-manual-form__row` (label+control stack, one per field) → `.add-manual-form__submit` (right-aligned action row). Rendered unconditionally on `/add`; see §Conventions "every web-UI write path must work with zero providers enabled". Ids are prefixed `add-manual-` to avoid collision when the provider forms coexist on the same page.
- **Banner variants (page-top, inline):** `.banner` is the neutral base; `.banner.error` (danger), `.banner.warn` (warn), `.banner.ok` (success) predate v0.3.3; `.banner.info` (accent, Session 23) is the informational variant used when a provider is disabled but the page still works. All four share the `color-mix(--semantic, --surface)` tint pattern; `.banner.info` pulls from `--accent`. Used via `showBanner(host, "info", …)` in `app.js` or `{{if …}}<div class="banner info">…</div>{{end}}` in templates.
...
**Non-goals for v0.3:** external book discovery; collaborative filtering; user-customizable rule weights; saved reading lists; automated reshelving; UI filters or sorting controls; LLM-generated explanations (that's v0.4).
...
> **2026-04-22 (Session 20 — v0.3.1):** The originally planned v0.4 reader-sync milestone was dropped from the roadmap. v0.5/v0.6/v0.7 renumbered to v0.4 (LLM) / v0.5 (Hardcover) / v0.6 (Mobile/Tailscale). The corresponding enum entry in `internal/domain/precedence`, the precedence test that covered it, and comments referencing it in precedence/mapper/pages/doc were all scrubbed. No behavioural changes.

### v0.3.2 — Web-UI shutdown (shipped 2026-04-22, 1 session)

> **2026-04-22:** Closes the browser-side gap in Shelf's shutdown story. The lifecycle already supports SIGINT/SIGTERM and the tray **Quit** menu item; a user whose primary surface is the PWA browser window had no way to stop the backend without reaching for the tray or a terminal. Landing shutdown here — before v0.4's LLM arc introduces the first opt-in outbound call — pairs the "big new capability" with a clear off switch.

**Non-goals for v0.3.2:** removing the tray Quit item (it stays — peer mechanism); building generic admin endpoints like delete-all or bulk-wipe (those will be separate work that inherits this session's destructive-endpoint hardening rule); auto-restart after shutdown; auth beyond CSRF + loopback (localhost-only invariant still holds).
...
### v0.3.3 — Offline-capable add-book (planned 2026-04-22; Sessions 23, 24)

> **2026-04-22:** The `/add` page (shipped v0.1 S6) is UI-gated on `providers.openlibrary.enabled = true`. With the provider disabled (the default since `0b54a34` strengthened the no-phone-home posture), `/add` renders "The Open Library provider is not configured. Add-book is disabled." and the user cannot add a book from the web UI at all — a regression against the local-first promise. The `AddCreate` handler itself does not require a provider; the block is purely at the template level. This mini-arc inserts between v0.4 open (S22) and v0.4 mid (S25) because blocking users from adding books while LLM tuning proceeds would be an obvious prioritisation mistake.

**Goals.** S23 ships a first-class manual-entry form on `/add` that works regardless of `ProviderWired`, downgrading the "disabled" error banner to an info-level note that points at `shelf.toml`. S24 ships drag-drop cover upload via a new `POST /api/covers/upload` (magic-byte validation, 5 MiB cap, JPEG/PNG only — SVG rejected) so manually-added books can carry art; the same dropzone also lets provider-flow users override a provider cover. Closes v0.3.3.

**Non-goals for v0.3.3.** Bulk ISBN/title paste (different UX, belongs in a future `/import`-style surface if needed); editing an existing book from `/add` (editing lives in Obsidian per Invariant #1; the PATCH endpoint on `/books/{filename}` exists for narrow cases); duplicate-detection UI beyond the existing 409 collision response; offline cache of past provider lookups; drag-drop beyond JPEG/PNG (no WebP, no AVIF, no SVG — the magic-byte surface stays small).

**Session 23 landed 2026-04-22** — opens v0.3.3. `internal/http/templates/add.html` restructured: the `{{if not .ProviderWired}}<div class="banner error">Add-book is disabled.</div>{{else}}…{{end}}` hard gate that dated to v0.1 S6 is gone. The page now renders (a) an optional `<div class="banner info">` when `!ProviderWired` that explains the state and points at `[providers.openlibrary]` in `shelf.toml`, (b) an always-present `<section class="add-manual">` carrying a first-class `#add-manual-form` with eleven labelled fields (`title*`, `subtitle`, `authors*` comma-separated, `isbn`, `format` select (`""`/`audiobook`/`ebook`/`physical`), `publisher`, `publish_date`, `total_pages`, `series`, `series_index`, `categories` comma-separated), and (c) the original ISBN-lookup + title-search forms plus `#add-results` now wrapped in `{{if .ProviderWired}}` so they only render when a `metadata.Provider` is wired. Every new form control has an id prefixed `add-manual-` to avoid collision with provider-form ids (`#isbn-input`, `#query-input`), and every `<input>`/`<select>` is paired with a matching `<label for=…>` so the existing `TestFormsUseExplicitLabelAssociation` continues to pass across all 11 new controls. **JS** (`app.js`): new `initAddManual()` binds `#add-manual-form` submit; parses comma-separated `authors` + `categories` into arrays (trim + drop empty); promotes `total_pages` and `series_index` to numbers only when the field is non-empty (so no spurious `null`s on the wire); client-side required-field guard on title + ≥1 author; reuses the existing `submitCreate(payload, anchor)` path (app.js:1011) so 409→warn, 400/500→error, and success-redirect to `response.data.url` stay identical to the provider flow. `cover: ""` in every manual payload makes `submitCreate` skip `resolveCover` — the manual path emits zero outbound provider traffic. `initAddPage()` calls `initAddManual()` before its existing `#add-results` null-check early-return, so both forms wire on a single DOMContentLoaded pass when both are present. **CSS** (`app.css`): new `.banner.info` variant tinted from `--accent` (mirroring the `.warn`/`.error`/`.ok` color-mix pattern — design-system cool-minimal palette retained); new `.add-manual`, `.add-manual-form`, `.add-manual-form__row`, `.add-manual-form__submit` classes — column-stacked label/control pairs with a right-aligned submit, token-driven padding/borders, zero inline styles. **Handler** (`internal/http/handlers/add.go`): no code change — `addCreateRequest` already treated every field except `Title` and `Authors` as optional, and the write path (`note.Create` + atomic + `Syncer.Apply`) is provider-agnostic. Verified by reading the file end-to-end; a nil `d.Covers` cannot be dereferenced because the `req.Cover != ""` gate at line 274 is not tripped by manual payloads (which always send `cover: ""`). The pre-existing ignore-untrusted-ref path at lines 274–285 is unchanged. **sw.js** `CACHE_VERSION` bumped `shelf-v11 → shelf-v12` so returning clients install the new template + CSS bundle. **Regression guards (8 new):** templates — `TestAddPageRendersManualFormAlways` (table over `ProviderWired ∈ {true, false}`), `TestAddPageHidesProviderFormsWhenDisabled`, `TestAddPageShowsInfoBannerWhenProviderDisabled` (asserts `.banner.info` present, the legacy "Add-book is disabled" copy absent, and no `.banner.error` leak), `TestAddPageShowsBothFormsWhenProviderEnabled`; handlers — `TestAddCreate_ManualMinimalPayload_WritesNote`, `TestAddCreate_ManualFullPayload_WritesAllFields` (asserts every supplied value survives YAML round-trip; tolerates encoder-dependent quoting by checking values, not quoted forms), `TestAddCreate_ManualPayload_TriggersReindex` (LibraryView sees the new book without a restart); integration — `TestEndToEnd_AddBookManualFlow` in `tests/integration/addbook_test.go` (reuses the existing `buildServer` helper, which has always omitted `Metadata`/`Covers` from Dependencies — the nil-provider scenario is the default, not a new test fixture). All four security lints (`go vet`, staticcheck 0.7.0, gosec, govulncheck) clean; `cmd/a11y-check` reports all blocking pairs PASS both palettes (the new `.banner.info` uses `--accent` on `color-mix(--accent 12%, --surface)` which is already covered by the curated `accent`-on-`surface` pair). **Deferred to S24 (v0.3.3 close):** drag-drop cover upload via `POST /api/covers/upload` with magic-byte validation (5 MiB cap, JPEG/PNG only, SVG rejected), `.cover-dropzone` + `.cover-dropzone--active` + `.cover-dropzone__preview` CSS classes, `icon-upload` sprite symbol, and the ability to override a provider-fetched cover with a local upload.

### v0.4 — LLM-enhanced recommender (open 2026-04-22; Sessions 22, 25, 26)

> **2026-04-22:** Roadmap expanded from the renumbered one-line stub into a three-session arc mirroring v0.2 and v0.3 cadence. Opt-in, bring-your-own Anthropic API key. Composes with the rule-based scorer from v0.3 S18, not replacing it — tunes the six scorer weights + axis preference targets from rated review text. Sends only user-provided text + structured metadata; no telemetry. **Session numbering.** S22 landed the client scaffolding; v0.3.3 (S23, S24) interrupts for offline add-book; v0.4 resumes at S25 (algorithm + endpoint) and closes at S26 (UI + chord).

**Goals.** Land a safe-by-default Anthropic HTTP client (S22 ✓), the tune algorithm + `/api/recommendations/tune` endpoint (S25), and the `/recommendations` "Tune with LLM" UI + `g t` chord (S26). At the end of the arc, a user with a valid API key can click one button to re-rank their recommendations based on their own review prose; the default behaviour of everyone else is byte-for-byte identical to v0.3.
...
### v0.6 — Mobile/Tailscale access (future)
...
- **Every web-UI write path must work with zero providers enabled.** Provider paths are enhancements, not prerequisites. A disabled provider renders an info banner and an alternate path, never a hard-block error banner. Rationale: the shipped default has all external providers disabled (Invariant #8, no-phone-home posture), so any write flow gated on a provider ships broken in the default configuration. `/add` is the canonical example — v0.1 S6 shipped it provider-gated, and S23 (2026-04-22) restructured the template so the manual-entry form is always rendered and the disabled state degrades to a `.banner.info` pointing at `shelf.toml`. Added 2026-04-22 (Session 23).
