...
Last updated: 2026-04-19 (v0.2.1 Session 15 complete — Trial-System rating: frontmatter `rating` parses legacy scalar + new map shape, serializes the map; `type Rating struct { TrialSystem, Overall }` with `Effective()`/`IsDimensioned()`/`HasOverride()` helpers; body `## Rating` section is dual-written from frontmatter on every write; `.rating-grid` widget replaces the single-star row with five axes + override controls; `icon-star-half` sprite symbol added (catalogue = 13); PATCH accepts the new shape only; Goodreads maps `My Rating` to `Rating{Overall}`; stats page adds rating-distribution histogram; `sw.js` cache bumped `shelf-v7 → shelf-v8`.)
