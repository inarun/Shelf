...
Last updated: 2026-04-19 (v0.2 Session 14 complete — v0.2 milestone closed: sync UI + audio timeline badge landed; `/sync` SSR page renders empty-state when AB disabled and plan/apply flow when wired; `icon-audio` sprite symbol + `timeline-source-icon` badge on book-detail entries matched against body "(Audiobookshelf)" markers; `sw.js` cache bumped `shelf-v6 → shelf-v7`)
